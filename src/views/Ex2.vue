<template>
  <div class="card-container">
    ...
    <div class="btn-footer">
      ...
      >
      <router-link to="/ex3" :class="{ 'non-clickable': !selectedChoice }">
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedChoiceFromEx1 = ref(null);

    if (store.getters.getSelectedChoice) {
      selectedChoiceFromEx1.value = store.getters.getSelectedChoice;
    }

    console.log("selectedChoiceFromEx1", selectedChoiceFromEx1.value);

    const choices = [
      { id: "choice_1", label: "การทำงาน", value: 30 },
      { id: "choice_2", label: "เที่ยวต่างประเทศ", value: 10 },
      { id: "choice_3", label: "ใช้ในชีวิตประจำวัน", value: 20 },
    ];
    ...
    const selectChoice = (choiceValue: number) => {
      ...
      // totalValue.value = selectedChoiceFromEx1.value! + selectedChoice.value!;
      if (
        selectedChoiceFromEx1.value !== null &&
        selectedChoice.value !== null
      ) {
        totalValue.value = selectedChoiceFromEx1.value + selectedChoice.value;
        store.dispatch("updateSelectedChoice2", totalValue);
